...
for k = 1:NParams
    figure(k);
    ek = errorbar(log10(abs(fdotvec_sig)), data(:, k), uncertainties(:, k), '.k');
...
for k = 1:NParams
    [fitParamfitParams, dParams] = fitChiSquare(fdotvec_sig, data(:, k), model_func, -rand(1, 2), zeros(size(fdotvec_sig)), uncertainties(:, k));
    %{
    ...
    figure(k);
    hold on;
    pk = plot(log10(abs(fdot_sigp)), model_func(fitParamfitParams, fdot_sigp), '-r');
