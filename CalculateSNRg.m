...
for r = 1:length(fvec)
    for j = 1:length(fdotvec)
        ...
        SNR1Array(j*r) = abs(signalDS1 - mean(noiseDS1))/std(noiseDS1);
        for i = M:N
            if (ParsevalSNR == 0)
...
for i = M:N
    if (strcmp(TS, 'f'))
        ...
        templateSpacings(i-M+1) = mode(abs(dfp(abs(SNRsplines(i-M+1, :) - 0.8*SNRg_iArray(i-M+1, searchScale+1)) == min(abs(SNRsplines(i-M+1, :) - 0.8*SNRg_iArray(i-M+1, searchScale+1))))));
    else
