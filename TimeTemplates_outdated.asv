...
for k = 1:numel(times)
    ...
    times{k}.TimeDomainSignal = sum(execLines(logical((201 <= execLines(:, 1)).*(execLines(:, 1) <= 203)), 3));
    times{k}.SegmentAndFFT = sum(execLines(logical((206 <= execLines(:, 1)).*(execLines(:, 1) <= 213)), 3));
    times{k}.CalcfIndex = sum(execLines(logical((216 <= execLines(:, 1)).*(execLines(:, 1) <= 224)), 3));
    times{k}.CalcWeights = sum(execLines(logical((227 <= execLines(:, 1)).*(execLines(:, 1) <= 231)), 3));
    times{k}.CalcDS = sum(execLines(logical((233 <= execLines(:, 1)).*(execLines(:, 1) <= 268)), 3));
    times{k}.CalcSNR = sum(execLines(logical((271 <= execLines(:, 1)).*(execLines(:, 1) <= 277)), 3));
end
