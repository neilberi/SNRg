...
if (TwoFitSigma == 1)
    ...
    for k = 1:length(Tobsvec)
        ...
        xlabel('Base 10 log of i');
        if (strcmp(TS, 'f'))
            ylabel('Base 10 log of Template Spacing (log(Hz))');
        else
            ylabel('Base 10 log of Template Spacing (log(Hz/s))');
        end
...
if (OutputFile == 1)
    if (strcmp(TS, 'f'))
        if (ParsevalSNR == 1)
            filenameOut = sprintf('fTemplateSpacingFitCoeffsfitDegree_%fP.csv', fitDegree);
        else 
            filenameOut = sprintf('fTemplateSpacingFitCoeffsfitDegree_%f.csv', fitDegree);
        end
        fout = fopen(filenameOut, 'a');
        fprintf(fout, '%e, ', fdot_sig);
    else
        if (ParsevalSNR == 1)
            filenameOut = sprintf('fdotTemplateSpacingFitCoeffsfitDegree_%fP.csv', fitDegree);
        else 
            filenameOut = sprintf('fdotTemplateSpacingFitCoeffsfitDegree_%f.csv', fitDegree);
        end
        fout = fopen(filenameOut, 'a');
        fprintf(fout, '%e, ', fdot_sig);
    end
